...
void main() {
    ...
    vec3 col = background;
    col = mix(col, background, smoothstep(0.0,  0.55, v));
    col = mix(col, text, smoothstep(0.5,  0.85, v));
    col = mix(col, secondaryText, smoothstep(0.75, 1.0,  v) * 0.55);
    col = mix(col, line, smoothstep(0.88, 1.0,  v) * 0.4);
